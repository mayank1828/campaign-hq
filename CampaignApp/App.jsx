 ...
 * * FOR PREVIEW: Uses the internal __firebase_config.
 * FOR PRODUCTION (Vercel/GitHub): You will need to switch to import.meta.env
 */
const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : {};

// To deploy to Vercel/GitHub, replace the line above with:
/*
...
// Initialize Firebase
// We add a check to prevent crashing if config is empty during setup
...
// --- HELPER COMPONENT: BOUNCING BADGE WITH OBSTACLE COLLISION ---
const BouncingBadge = ({ obstacleRef }) => {
  const badgeRef = useRef(null);
  const requestRef = useRef();
  
  // Physics state - Spawn in top-left to be safe initially
  const state = useRef({
    x: 20,
    y: 20,
    vx: 1.8, // Slightly faster for better effect
    vy: 1.8 
  });
  ...
  useEffect(() => {
    const animate = () => {
      ...
      // 1. Screen Boundary Collision (Bounce off walls)
      if (nextX <= 0) {
        nextX = 0;
        nextVx = Math.abs(vx);
      } else if (nextX + rect.width >= innerWidth) {
        nextX = innerWidth - rect.width;
        nextVx = -Math.abs(vx);
      }
      ...
      if (nextY <= 0) {
        nextY = 0;
        nextVy = Math.abs(vy);
      } else if (nextY + rect.height >= innerHeight) {
        nextY = innerHeight - rect.height;
        nextVy = -Math.abs(vy);
      }

      // 2. Obstacle Collision (Bounce off ONLY the Login Card)
      if (obstacleRef && obstacleRef.current) {
        const obs = obstacleRef.current.getBoundingClientRect();
        
        // Define Next Frame Bounding Box for the Badge
        ...
        // Check Intersection with Obstacle (The Login Card)
      ...
      // Auto Tilt based on movement direction
      const tiltX = nextVy * -5; // Tilt up/down
      const tiltY = nextVx * 5;  // Tilt left/right
    ...
    >
      <div className="relative group">
        ...
        <div className="relative bg-neutral-900/90 backdrop-blur-xl border border-amber-500/50 rounded-full px-8 py-3 shadow-[0_0_30px_rgba(245,158,11,0.3)] flex flex-col items-center min-w-[280px]">
          ...
          <span className="text-[10px] font-bold text-amber-500/80 tracking-[0.3em] uppercase">
            A Product by MAYANK
          </span>
        </div>
      </div>
...
// 1. LOGIN SCREEN - ROYAL BLACK & GOLD EDITION
const Login = ({ onLogin }) => {
  const [mode, setMode] = useState('volunteer'); // 'admin' or 'volunteer'
  const [name, setName] = useState('');
  const [pin, setPin] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  
  // Ref for the specific Login Card part (The Obstacle)
  ...
  return (
    <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden selection:bg-amber-500/30">
      ...
      {/* RICH AMBIENCE - Gold Dust Effect */}
      ...
      {/* AUTOMATIC BOUNCING BADGE (Collides with loginCardRef) */}
      <BouncingBadge obstacleRef={loginCardRef} />

      {/* LOGIN CONTAINER */}
      <div className="w-full max-w-md relative z-10 flex flex-col items-center">
        
        {/* Logo Section - NO COLLISION HERE (Badge flies over this) */}
        ...
        {/* Login Card - THIS IS THE OBSTACLE */}
        <div className="w-full" ref={loginCardRef}>
          <div className="bg-neutral-900/80 backdrop-blur-md rounded-[20px] p-8 relative overflow-hidden border border-amber-500/20 shadow-[0_0_50px_-10px_rgba(245,158,11,0.1)]">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input 
                      type="text" 
                      required 
                      className="w-full pl-11 pr-4 py-4 bg-neutral-950 border border-neutral-800 rounded-xl focus:ring-1 focus:ring-amber-500/50 focus:border-amber-500/50 outline-none transition-all font-medium text-amber-50 placeholder:text-neutral-700"
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input 
                    type="password" 
                    required 
                    className="w-full pl-11 pr-4 py-4 bg-neutral-950 border border-neutral-800 rounded-xl focus:ring-1 focus:ring-amber-500/50 focus:border-amber-500/50 outline-none transition-all font-medium text-amber-50 placeholder:text-neutral-700 tracking-widest"
              ...
              <button 
                disabled={loading}
                className={`w-full font-bold py-4 px-4 rounded-xl transition-all shadow-xl flex justify-center items-center gap-2 mt-6 transform hover:scale-[1.02] active:scale-[0.98]
                  ${mode === 'volunteer' 
                    ? 'bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600 text-neutral-950 shadow-amber-500/20' 
                    : 'bg-gradient-to-r from-red-900 via-red-700 to-red-900 text-white shadow-red-900/30 border border-red-500/30'}`}
              >
                {loading ? <RefreshCw className="animate-spin h-5 w-5" /> : (
                  <>
                    <span className="text-lg tracking-wide uppercase font-black">Enter System</span>
                    <Zap className={`h-5 w-5 ${mode === 'volunteer' ? 'fill-neutral-900' : 'fill-white'}`} />
                  </>
                )}
              </button>
            </form>
...
// 2. ADMIN DASHBOARD - RICH DARK MODE
const AdminDashboard = ({ currentUser, onLogout }) => {
  ...
  useEffect(() => {
    ...
    // Listen to Contact Stats
    const unsubContacts = onSnapshot(
      collection(db, 'artifacts', appId, 'public', 'data', CONTACTS_COLLECTION),
      (snapshot) => {
  ...
  const deleteVolunteer = async (id) => {
    if(confirm('Delete this volunteer?') && db) {
  ...
  const handleCsvUpload = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Error uploading. Ensure format is Name,Phone per line.');
    } finally {
  ...
  const distributeContacts = async () => {
    if (volunteers.length === 0 || !db) return alert("No volunteers to assign to!");
    setIsProcessing(true);
    try {
      const q = query(
        collection(db, 'artifacts', appId, 'public', 'data', CONTACTS_COLLECTION),
      ...
      const snapshot = await getDocs(q);
      if (snapshot.empty) {
        alert("No unassigned contacts left!");
        setIsProcessing(false);
        return;
      }
      const batch = writeBatch(db);
  ...
  const clearAllContacts = async () => {
    if (!confirm("DANGER: This will delete ALL 5000 contacts. Are you sure?") || !db) return;
    setIsProcessing(true);
    ...
    <div className="min-h-screen bg-neutral-950 font-sans text-amber-50">
      ...
      <div className="bg-neutral-900 border-b border-amber-900/30 px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-lg shadow-black/50">
        <h1 className="text-xl font-bold flex items-center gap-2">
          ...
          Campaign<span className="text-amber-400">HQ</span>
        </h1>
        <div className="flex items-center gap-4">
          <span className="text-xs text-amber-500 hidden md:inline bg-amber-950/50 px-3 py-1 rounded-full border border-amber-900/50 tracking-widest font-bold">ADMIN MODE</span>
          <button onClick={onLogout} className="text-neutral-400 hover:text-red-400 hover:bg-red-950/30 p-2 rounded-lg transition">
        ...
        <div className="lg:col-span-1 space-y-2">
          {['stats', 'volunteers', 'contacts'].map((tab) => (
            <button 
              ...
              className={`w-full text-left px-4 py-3.5 rounded-xl flex items-center gap-3 font-bold transition-all duration-200 capitalize ${
                activeTab === tab 
                ? 'bg-gradient-to-r from-amber-600 to-yellow-600 text-neutral-900 shadow-lg shadow-amber-500/20 scale-[1.02]' 
                : 'bg-neutral-900 text-neutral-500 hover:bg-neutral-800 border border-neutral-800 hover:text-amber-400'
              }`}
            >
              {tab === 'stats' && <BarChart3 className="h-5 w-5" />}
              {tab === 'volunteers' && <Users className="h-5 w-5" />}
              {tab === 'contacts' && <Upload className="h-5 w-5" />}
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="lg:col-span-3">
          {activeTab === 'stats' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-neutral-900 p-6 rounded-2xl shadow-lg border border-amber-900/30 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 w-24 h-24 bg-amber-500/10 rounded-bl-full -mr-4 -mt-4 transition group-hover:scale-110"></div>
                  <p className="text-amber-600 text-sm font-bold uppercase tracking-wider relative z-10">Total Contacts</p>
                  <p className="text-4xl font-black text-white mt-2 relative z-10">{stats.total}</p>
                </div>
                <div className="bg-neutral-900 p-6 rounded-2xl shadow-lg border border-amber-900/30 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 w-24 h-24 bg-orange-500/10 rounded-bl-full -mr-4 -mt-4 transition group-hover:scale-110"></div>
                  <p className="text-orange-600 text-sm font-bold uppercase tracking-wider relative z-10">Pending</p>
                  <p className="text-4xl font-black text-white mt-2 relative z-10">{stats.total - stats.sent}</p>
                </div>
                <div className="bg-neutral-900 p-6 rounded-2xl shadow-lg border border-amber-900/30 relative overflow-hidden group">
                  ...
                  <p className="text-green-600 text-sm font-bold uppercase tracking-wider relative z-10">Sent</p>
                  <p className="text-4xl font-black text-white mt-2 relative z-10">{stats.sent}</p>
                </div>
              </div>
              ...
              <div className="bg-neutral-900 p-8 rounded-2xl shadow-xl border border-amber-900/20">
                ...
                <div className="flex flex-wrap gap-4">
                  <button 
                    onClick={distributeContacts}
                    disabled={isProcessing}
                    className="flex items-center gap-2 bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-500 hover:to-yellow-500 text-neutral-950 px-8 py-4 rounded-xl font-bold shadow-lg shadow-amber-900/40 transition transform hover:-translate-y-1"
                  >
                    <Share2 className="h-5 w-5" />
                    {isProcessing ? 'Processing...' : 'Distribute Contacts'}
                  </button>
                  <button 
                    onClick={clearAllContacts}
                    className="flex items-center gap-2 text-red-500 hover:bg-red-950/30 px-8 py-4 rounded-xl font-bold border border-red-900/50 transition"
                  >
                    <Trash2 className="h-5 w-5" /> Clear Data
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
               <div className="bg-neutral-900 p-6 rounded-2xl shadow-lg border border-amber-900/30">
                <h3 className="text-lg font-bold text-amber-50 mb-4 flex items-center gap-2">
                  <UserPlus className="h-5 w-5 text-amber-500" /> Add New Volunteer
                </h3>
                <form onSubmit={addVolunteer} className="flex gap-4 items-end">
                  <div className="flex-1">
                    ...
                    <input 
                      className="w-full bg-neutral-950 border border-neutral-800 p-3 rounded-lg mt-1 focus:ring-1 focus:ring-amber-500 outline-none font-medium text-amber-50 placeholder:text-neutral-700" 
                      placeholder="e.g. Rahul Sharma"
                      value={newVolName}
                  ...
                  <div className="w-32">
                    <label className="text-xs font-bold text-neutral-500 uppercase">PIN</label>
                    <input 
                      className="w-full bg-neutral-950 border border-neutral-800 p-3 rounded-lg mt-1 focus:ring-1 focus:ring-amber-500 outline-none font-medium text-amber-50 placeholder:text-neutral-700" 
                      placeholder="4-digit"
                      value={newVolPin}
                      onChange={e => setNewVolPin(e.target.value)}
                    />
                  </div>
               ...
               <div className="grid gap-3">
                 {volunteers.map(vol => (
                   <div key={vol.id} className="bg-neutral-900 p-4 rounded-xl shadow-sm border border-neutral-800 flex justify-between items-center group hover:border-amber-900/50 transition">
                     <div className="flex items-center gap-4">
                       <div className="h-10 w-10 bg-gradient-to-br from-amber-700 to-yellow-600 rounded-full flex items-center justify-center text-neutral-950 font-bold">
                         {vol.name.charAt(0)}
                       </div>
                       <div>
                         <p className="font-bold text-amber-50">{vol.name}</p>
                         <p className="text-xs text-neutral-500 font-mono bg-neutral-950 inline-block px-2 py-0.5 rounded border border-neutral-800">PIN: {vol.pin}</p>
                       </div>
                     </div>
                     <button onClick={() => deleteVolunteer(vol.id)} className="text-neutral-600 hover:text-red-500 transition">
          ...
          {activeTab === 'contacts' && (
            <div className="bg-neutral-900 p-8 rounded-2xl shadow-lg h-full border border-amber-900/30">
              <h3 className="text-lg font-bold text-amber-50 mb-2">Bulk Upload Contacts</h3>
              <p className="text-sm text-neutral-500 mb-4">Paste CSV data: <code>Name,Phone</code> (one per line)</p>
              <textarea 
                className="w-full h-64 p-4 bg-neutral-950 border border-neutral-800 rounded-xl font-mono text-sm focus:ring-1 focus:ring-amber-500 outline-none resize-none text-amber-50 placeholder:text-neutral-700"
                placeholder="Amit Kumar, +919876543210&#10;Priya Singh, +919123456789"
                value={csvText}
              ...
              <div className="mt-6 flex justify-end">
                <button 
                  ...
                  className="bg-amber-600 text-neutral-950 px-8 py-3 rounded-xl font-bold shadow-lg hover:bg-amber-500 disabled:opacity-50 transition"
                >
...
// 3. VOLUNTEER DASHBOARD - GOLD & BLACK THEME
const VolunteerDashboard = ({ currentUser, onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (storedCooldown && new Date(storedCooldown) > new Date()) {
      setCooldownUntil(new Date(storedCooldown));
    }
  ...
  const handleMarkSent = async (contactId) => {
    ...
    if (newCount >= 10) {
      const finishTime = new Date(new Date().getTime() + 10 * 60000); // 10 minutes
  ...
  const getWhatsappLink = (contact) => {
    const message = `Hello ${contact.name}, please support us in the upcoming election! Your vote counts.`;
  ...
  return (
    <div className="min-h-screen bg-neutral-950 pb-10 font-sans text-amber-50">
      <div className="bg-neutral-900 border-b border-amber-900/30 px-6 py-4 shadow-lg sticky top-0 z-10">
        <div className="max-w-3xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="font-black text-xl text-white tracking-wide">Volunteer Portal</h1>
            <p className="text-amber-500 text-xs font-bold bg-amber-950/50 border border-amber-900/50 inline-block px-3 py-1 rounded-full mt-1">Hello, {currentUser.name}</p>
          </div>
          <button onClick={onLogout} className="bg-white/5 p-2 rounded-lg hover:bg-white/10 transition backdrop-blur-sm border border-white/5">
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-6 mt-4">
        ...
        {isCooldownActive ? (
           <div className="bg-neutral-900 p-8 rounded-3xl shadow-2xl border border-amber-500/30 text-center animate-fade-in relative overflow-hidden">
             ...
             <div className="w-20 h-20 bg-neutral-950 rounded-full flex items-center justify-center mx-auto mb-6 border border-amber-900/50 shadow-[0_0_30px_rgba(245,158,11,0.2)]">
               <Clock className="h-10 w-10 text-amber-500" />
             </div>
             <h2 className="text-2xl font-black text-white mb-2 tracking-wide">Safety Break Active</h2>
             <p className="text-neutral-400 mb-6 max-w-md mx-auto">
               System cooling down to protect the sender ID.
             </p>
             <div className="text-5xl font-mono font-bold text-amber-500 tracking-wider drop-shadow-lg">
               {minutes}:{seconds.toString().padStart(2, '0')}
             </div>
             <p className="text-xs font-bold text-amber-700 uppercase tracking-[0.3em] mt-6">System Locked</p>
           </div>
        ) : (
          <>
            {/* Progress Card */}
            <div className="bg-neutral-900 p-6 rounded-2xl shadow-xl border border-amber-900/30 flex justify-between items-center relative overflow-hidden">
              ...
              <div className="relative z-10">
                <p className="text-amber-600 text-xs font-bold uppercase tracking-wider mb-1">Batch Progress</p>
                <div className="flex items-end gap-2">
                   <h2 className="text-4xl font-black text-white">{sessionSentCount}<span className="text-neutral-600">/</span>10</h2>
                   <span className="text-sm font-bold text-neutral-500 mb-1.5">sent</span>
                </div>
              </div>
              <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-amber-600 to-yellow-600 shadow-lg shadow-amber-500/20 flex items-center justify-center text-neutral-950 font-black text-xl relative z-10">
                {totalPending}
              </div>
            </div>
            ...
            <div className="space-y-3">
              {loading ? (
                <p className="text-center text-neutral-500 mt-10 animate-pulse font-mono">Loading encrypted data...</p>
              ) : totalPending === 0 ? (
                <div className="text-center py-12 bg-neutral-900 rounded-2xl shadow-sm border border-neutral-800">
                  <CheckCircle className="h-16 w-16 text-green-500 mx-auto mb-4" />
                  <h3 className="text-xl font-bold text-white">Mission Complete!</h3>
                  <p className="text-neutral-500 mt-2">All tasks finished successfully.</p>
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-2 mb-2 px-1 opacity-70">
                     <AlertTriangle className="h-4 w-4 text-amber-500" />
                     <p className="text-xs font-bold text-amber-500 uppercase tracking-wide">Next {visibleContacts.length} Contacts</p>
                  </div>
                  {visibleContacts.map(contact => (
                    <div 
                      key={contact.id} 
                      className="bg-neutral-900 p-5 rounded-2xl shadow-lg border border-neutral-800 flex items-center justify-between transition-all duration-300 hover:border-amber-500/30 hover:shadow-amber-500/10 group"
                    >
                      <div>
                        <h4 className="font-bold text-lg text-white group-hover:text-amber-100 transition">{contact.name}</h4>
                        <p className="text-sm text-neutral-500 font-mono tracking-wide">{contact.phone}</p>
                      </div>
                      ...
                      <div className="flex gap-3">
                        <a 
                          ...
                          target="_blank" 
                          rel="noreferrer"
                          className="bg-[#25D366] hover:bg-[#20bd5a] text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-green-900/20 transition hover:scale-105"
                        >
                          <MessageCircle className="h-5 w-5" /> WhatsApp
                        </a>
                        <button 
                          onClick={() => handleMarkSent(contact.id)}
                          className="bg-neutral-800 hover:bg-neutral-700 text-neutral-400 hover:text-white px-4 py-2.5 rounded-xl transition border border-neutral-700"
                          title="Mark as Sent"
                        >
